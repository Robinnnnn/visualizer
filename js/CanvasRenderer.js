...
THREE.CanvasRenderer = function(parameters) {
	...
	this.setBackgroundColor = function(backgroundColor) {
		_clearColor = new THREE.Color(backgroundColor)
	}
	...
	this.clear = function() {
		...
		if (_clearBox.empty() === false) {
			...
			if (_clearAlpha < 1) {
				...
				_context.clearRect(
					_clearBox.min.x | 0,
					_clearBox.max.y | 0, (_clearBox.max.x - _clearBox.min.x) | 0, (_clearBox.min.y - _clearBox.max.y) | 0
				);
	...
	function textureToPattern(texture) {
		...
		_patterns[texture.id] = _context.createPattern(
			canvas, repeatX === true && repeatY === true ? 'repeat' : repeatX === true && repeatY === false ? 'repeat-x' : repeatX === false && repeatY === true ? 'repeat-y' : 'no-repeat'
		);
